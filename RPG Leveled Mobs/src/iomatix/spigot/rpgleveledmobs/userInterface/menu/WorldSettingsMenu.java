			...
			public void generateMenu() {
				...
				for (final SpawnNode node : this.nodes) {
					...
					nodeButton.addLoreLine(ChatColor.WHITE + "X: " + ChatColor.LIGHT_PURPLE + node.getLocation().getX()
							...
							+ ChatColor.WHITE + " Z: " + ChatColor.LIGHT_PURPLE + node.getLocation().getZ());
				...
				createButton.setOnPressedListener(new Button.onButtonPressedListener() {
					...
					public void onButtonPressed(final MenuInteractionEvent event) {
						...
						final SpawnNode node = ((WorldConfig) WorldSettingsMenu.this.config).addSpawnNode(loc.getX(),
								loc.getY(), loc.getZ());
				...
				if (this.next == null && this.prev != null) {
					...
					for (beginning = this.prev; beginning != null
							&& beginning.getPrev() != null; beginning = beginning.getPrev()) {
					}
