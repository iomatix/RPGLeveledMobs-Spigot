	...
	protected class LeveledMenu extends Menu {
		...
		public void generateNewMenu(int page) {
			if (page == 0) {
				...
				final Button CaveSpider = new MobButton("Cave Spider", EntityType.CAVE_SPIDER, SettingsMenu.this.config,
						this);
			...
			prefixFormat.setOnPressedListener(new Button.onButtonPressedListener() {
				...
				public void onButtonPressed(final MenuInteractionEvent event) {
					...
					try {
						...
						event.getInteractor()
								.sendMessage(ChatColor.YELLOW + "Please enter a new value, use # for the mob's level.");
					} catch (NoSuchMethodException e) {
						e.printStackTrace();
					}
				}
