...
public class MenuHandler implements Listener {
	...
	public void onInventoryInteraction(final InventoryClickEvent event) {
		if (MenuHandler.menuHashMap.containsKey(event.getClickedInventory())) {
			...
			try {
				event.setCancelled(true);
				MenuHandler.menuHashMap.get(event.getClickedInventory()).OnMenuInteraction(
						new MenuInteractionEvent(event, MenuHandler.menuHashMap.get(event.getClickedInventory())));
	...
	public void onItemMoveEvent(final InventoryMoveItemEvent event) {
		if (MenuHandler.menuHashMap.containsKey(event.getDestination())
				|| MenuHandler.menuHashMap.containsKey(event.getInitiator())
				|| MenuHandler.menuHashMap.containsKey(event.getSource())) {
			event.setCancelled(true);
		}
	}
