...
public class NearestSpawnCommand implements RPGlvlmobsCommand {
    ...
    public boolean execute(final CommandSender sender, final ArrayList<String> args) {
        ...
        sender.sendMessage(ChatColor.GREEN + worldName + " - " + ChatColor.YELLOW + node.getName() + " - " + ChatColor.BLUE + this.formatter.format(node.getLocation().distance(((Player)sender).getLocation())) + " Blocks");
        return true;
    }
