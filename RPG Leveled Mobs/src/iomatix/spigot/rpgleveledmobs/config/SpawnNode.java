...
public class SpawnNode extends RPGLeveledMobsConfig
{
    ...
    private SpawnNode(final ConfigurationSection nodeSection, final WorldConfig worldConfig) {
        ...
        this.loc = new Location(worldConfig.getWorld(), this.nodeConfig.getDouble("Location.x", 0.0), this.nodeConfig.getDouble("Location.y", 0.0), this.nodeConfig.getDouble("Location.z", 0.0));
        worldConfig.saveNodeConfig();
        this.inheritedValues = worldConfig.getSettings();
    }
    
    public static SpawnNode CreateSpawnNode(final WorldConfig wc, final World world, final double x, final double y, final double z) {
        for (final SpawnNode tempNode : SpawnNode.spawnNodes.get(wc)) {
            if (tempNode.getLocation().getX() == x && tempNode.getLocation().getY() == y && tempNode.getLocation().getZ() == z) {
                return tempNode;
            }
        }
    ...
    public static SpawnNode CreateSpawnNode(final ConfigurationSection section, final WorldConfig config) {
        for (final SpawnNode tempNode : SpawnNode.spawnNodes.get(config)) {
            if (tempNode.getLocation().getX() == section.getDouble("Location.x") && tempNode.getLocation().getY() == section.getDouble("Location.y") && tempNode.getLocation().getZ() == section.getDouble("Location.z")) {
                return tempNode;
            }
        }
    ...
    public void setDefenseMultiplier(final double multiplier) {
        ...
        this.nodeConfig.set(ConfigKey.DAMAGE_PER_LEVEL.toString(), (Object)this.defenseMult);
        this.worldConfig.saveNodeConfig();
    }
    ...
    public EntityType getRandomAllowedMob() {
        ...
        int currentTries;
        int random;
        for (maxTries = 5, currentTries = 0, random = this.rand.nextInt(allowed.size()); blocked.contains(allowed.get(random)) && currentTries++ < maxTries; random = this.rand.nextInt(allowed.size())) {}
